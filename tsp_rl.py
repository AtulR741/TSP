...
Original file is located at
    https://colab.research.google.com/drive/1Wfzy7vB4qXDeYF1jSRY7IL5SMJ4kkTxV
"""
...
In the state, we have the weight matrix *W*, the node coordinates *coords*, and the list of visited nodes *partial_solution*. We also define the function *state2tens*, which translates such tuples into tensors.
"""
...
def state2tens(state):
    ...
    coords = state.coords
    nr_nodes = coords.shape[0] #shape = [n, 2]
...
class QNet(nn.Module):
    ...
    def forward(self, xv, Ws):
        ...
        s3_1 = F.relu(self.theta4(Ws.unsqueeze(3)))  # (batch_size, num_nodes, num_nodes, emb_dim)
        s3_2 = torch.sum(s3_1, dim=1)  # (batch_size, num_nodes, emb_dim)
        s3 = self.theta3(s3_2)  # (batch_size, num_nodes, emb_dim)
        ...
        global_state = self.theta6(torch.sum(mu, dim=1, keepdim=True).repeat(1, num_nodes, 1))  # (batch_dim, num_nodes, emb_dim)

        local_action = self.theta7(mu)  # (batch_dim, num_nodes, emb_dim)

        out = F.relu(torch.cat([global_state, local_action], dim=2))  # (batch_dim, num_nodes, 2*emb_dim)
        return self.theta5(out).squeeze(dim=2)  # (batch_dim, num_nodes)

"""#Q-Function as a Wrapper around the model"""
...
class QFunction():
    ...
    def get_best_action(self, state_tsr, state):
        ...
        """
        W = state.W
        estimated_rewards = self.predict(state_tsr, W)  # size (num_nodes)
...
We'll now a tuple representing an experience, and the memory that contains such experiences. An experience is composed of a (state, action) tuple, and the corresponding "next state" and reward.
"""
...
for episode in range(NR_EPISODES):
    ...
    while not is_state_final(current_state):
        ...
        # store our experience in memory
